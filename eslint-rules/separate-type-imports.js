                    ...
                    context.report({
                        fix(fixer) {
                            ...
                            if (valueSpecifiers.length > 0) {
                                ...
                                const namespaceImportPart = buildNamespaceImportPart(namespaceImports, defaultImports.length > 0);
                                if (namespaceImportPart) {
                                    importParts.push(namespaceImportPart);
                                }
                                // Add named imports with proper comma
                                const namedImportsPart = buildNamedImportsPart(namedImports, defaultImports.length > 0 || namespaceImports.length > 0);
                                if (namedImportsPart) {
