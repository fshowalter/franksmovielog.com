 ...
 */
function buildNamedSpecifier(spec: ImportSpecifierWithKind): string {
  const imported = spec.imported as ESTree.Identifier;
  ...
  create(context) {
    return {
      ImportDeclaration(node) {
        const importNode = node as ImportDeclarationWithKind;
        ...
        for (const specifier of importNode.specifiers) {
          if (specifier.type === "ImportSpecifier") {
            const importSpec = specifier as ImportSpecifierWithKind;
