...
type CreditedAsFacetItem = { creditedAs: string[]; title: string };
 ...
 */
export function creditedAsFilterFacetTests(
  renderItems: (items: CreditedAsFacetItem[]) => void,
  getList: () => HTMLElement,
): void {
  describe("creditedAs filter", () => {
    it("filters to single credit (OR logic, shows matching, hides others)", async ({
      expect,
    }) => {
      vi.useFakeTimers({ shouldAdvanceTime: true });

      const items: CreditedAsFacetItem[] = [
