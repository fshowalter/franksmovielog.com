...
const ActionTypes = {
  CHANGED: "directors/changed" as const,
};

export const STATE_KEY = "directors";

export type DirectorsFilterChangedAction = {
  type: typeof ActionTypes.CHANGED;
  values: readonly string[];
};

export function createDirectorsFilterChangedAction(
  values: readonly string[],
): DirectorsFilterChangedAction {
  return { type: ActionTypes.CHANGED, values };
}

export function directorsFacetReducer<
  TState extends { pendingFilterValues: { [STATE_KEY]?: readonly string[] } },
>(state: TState, action: { type: string }): TState {
  switch (action.type) {
    case ActionTypes.CHANGED: {
      const { values } = action as DirectorsFilterChangedAction;
      if (values.length === 0) {
        return {
          ...state,
