...
type DirectorsFacetItem = { title: string; watchlistDirectorNames: string[] };
 ...
 */
export function directorsFilterFacetTests(
  renderItems: (items: DirectorsFacetItem[]) => void,
  getList: () => HTMLElement,
): void {
  describe("directors filter", () => {
    it("filters to single director (OR logic, shows matching, hides others)", async ({
      expect,
    }) => {
      vi.useFakeTimers({ shouldAdvanceTime: true });

      const items: DirectorsFacetItem[] = [
