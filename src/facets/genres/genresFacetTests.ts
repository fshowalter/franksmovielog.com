 ...
 */
export function genresFilterFacetTests(
  renderItems: (items: GenresFacetItem[]) => void,
  getList: () => HTMLElement,
): void {
  describe("genres filter", () => {
    it("filters to single genre (OR logic, shows matching, hides others)", async ({
      ...
      await clickCheckboxListOption(user, "Genres", "Horror");
    ...
    it("filters to multiple genres (OR logic)", async ({ expect }) => {
      vi.useFakeTimers({ shouldAdvanceTime: true });
      ...
      await clickToggleFilters(user);
      await clickCheckboxListOption(user, "Genres", "Horror");
      await clickCheckboxListOption(user, "Genres", "Sci-Fi");
      await clickViewResults(user);
