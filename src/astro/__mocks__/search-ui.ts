...
export class SearchUI {
  ...
  private loadMoreWrapper: HTMLElement | null = null;
  ...
  async init(): Promise<void> {
    ...
    if (this.api && typeof this.api === "object" && "init" in this.api) {
      await (this.api as { init(): Promise<void> }).init();
    }
  ...
  private async handleSearch(query: string): Promise<void> {
    ...
    try {
      ...
      const searchResults = await (this.api as any).debouncedSearch(trimmedQuery);
  ...
  private async renderResults(): Promise<void> {
    ...
    for (const result of nextBatch) {
      const data = await (result as any).data();
      const resultHtml = `
  ...
  async destroy(): Promise<void> {
    ...
    if (this.api && typeof this.api === "object" && "destroy" in this.api) {
      await (this.api as { destroy(): Promise<void> }).destroy();
    }
